...
// Maximum number of events to keep in storage (prevent quota issues)
// chrome.storage.local has ~5MB quota, keeping this conservative
...
class OpenBDRLogger {
   ...
   * Handles quota errors by trimming old events
   */
    async flush() {
        try {
            ...
            if (e.message && e.message.includes('quota')) {
                console.warn('[OpenBDR] Storage quota exceeded, trimming events...');
                // Aggressively trim to half the max
                this.eventBuffer = this.eventBuffer.slice(-(MAX_EVENTS / 2));
                try {
                    await chrome.storage.local.set({ [STORAGE_KEY]: this.eventBuffer });
                    console.log('[OpenBDR] Events trimmed successfully');
                } catch (e2) {
                    console.error('[OpenBDR] Still cannot save, clearing storage:', e2);
                    this.eventBuffer = [];
