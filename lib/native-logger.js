...
class NativeLogger {
    ...
    connect() {
        return new Promise((resolve) => {
            try {
                console.log(`[OpenBDR] Connecting to native host: ${NATIVE_HOST_NAME}`);
                ...
                this.port.onDisconnect.addListener(() => {
                    ...
                    console.warn('[OpenBDR] Native host port disconnected:', error?.message || 'Host process exited');
                ...
                this.sendMessage({ type: 'PING' })
                    .then(async (response) => {
                        if (response && response.pong) {
                            ...
                            console.log('[OpenBDR] Native host handshake successful');

                            // Send session start
                            await this.sendSessionStart();

                            // Flush any events that were buffered while disconnected
                            if (this.eventBuffer.length > 0) {
                                console.log(`[OpenBDR] Flushing ${this.eventBuffer.length} buffered events to SQLite...`);
                                await this.flushBuffer();
                            }

                            resolve(true);
                        } else {
                            console.warn('[OpenBDR] Native host responded but ping failed');
                            this.connected = false;
                    ...
                    .catch((err) => {
                        console.error('[OpenBDR] Native host handshake timeout or error:', err.message);
                        this.connected = false;
                        this.status.connected = false;
                        resolve(false);
                    });
                ...
                console.error('[OpenBDR] Fatal error during native host connection:', e);
                this.connected = false;
                this.status.connected = false;
                resolve(false);
            }
